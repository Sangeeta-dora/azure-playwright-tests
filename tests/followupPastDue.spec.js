 ...
 */
test('Followup Past Due_Enable Preview Feature_ Click on Enable Preview Features', async ({ followupPastDuePage, page,followupPastDuePageReady}) => {
 ...
 */
test('Followup Past Due_Enable Preview Feature_ Click on Enable Preview Features _Refresh Page', async ({ followupPastDuePage, page,followupPastDuePageReady }) => {
 await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
...
test('Should apply filters to the data grid_ String', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {

  await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
  await followupPastDuePage.takeScreenshot('before-applying-filter');    // Use the dedicated method from the page object that handles the entire filtering test workflow
...
test('Should apply filters to the data grid_ String_TimeZoneID', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
...
test('Should apply filters to the data grid_ String_ActivityNote', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
...
test('Should apply filters to the data grid_ String_CustID', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
...
test('Should apply filters to the data grid_ Number_DaysToRespond', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  await followupPastDuePage.verifyPageLoaded();
  await followupPastDuePage.waitForGridDataToLoad();
...
test('Should select a row and verify grid selection state', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  // All login and navigation to Followup Past Due page is handled by the followupPastDuePageReady fixture
  ...
  // Check if there are rows in the grid using the page object
  const rowCount = await followupPastDuePage.dataGridRows.count();
  console.log(`Found ${rowCount} rows in the grid`);

  if (rowCount > 0) {
    // Select the first row in the data grid
    console.log('Selecting first row in the data grid...');
    await followupPastDuePage.selectRowByIndex(0);

    // Take screenshot after selection
    await followupPastDuePage.takeScreenshot('after-row-selection');

    // Verify row is selected by checking for the selected CSS class using page object
    // This verification method depends on the specific grid implementation
    const selectedRow = followupPastDuePage.dataGridRows.first();
    const isSelected = await selectedRow.getAttribute('aria-selected');
    console.log(`Row selected state: ${isSelected}`);

    // Verify the row is selected
    expect(isSelected).toBe('true');

    // Alternative verification: check for selection class using page object
    const selectedRowLocator = followupPastDuePage.page.locator('.MuiDataGrid-row.Mui-selected').first();
    const hasSelectionClass = await selectedRowLocator.isVisible().catch(() => false);

    if (hasSelectionClass) {
      console.log('Row selection confirmed by CSS class');
    } else {
      console.log('Row selection confirmed by aria-selected attribute');
    }

    // Check if row has checkbox for selection using page object
    const rowCheckbox = followupPastDuePage.dataGridRows.first().locator('input[type="checkbox"]');
    const checkboxExists = await rowCheckbox.isVisible().catch(() => false);

    if (checkboxExists) {
      await rowCheckbox.check();
      await followupPastDuePage.takeScreenshot('after-checkbox-selection');

      // Verify checkbox is checked
      const isChecked = await rowCheckbox.isChecked();
      console.log(`Row checkbox is checked: ${isChecked}`);
    }
  } else {
    console.log('No rows available in the grid for selection');
    await followupPastDuePage.takeScreenshot('no-rows-available');
  }
});
// Test for clicking on customer link
test('Should click on customer name and navigate to details', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  ...
  await followupPastDuePage.clickCustomerLink(0);
  ...
  // Verify we're on the customer details page - look for common elements
  const isOnDetailsPage = await followupPastDuePage.page.locator('.customer-details-header, .account-details-header, .account-info-section').isVisible();

  // If we didn't find the header, check URL or title to verify navigation
  if (!isOnDetailsPage) {
    const currentUrl = followupPastDuePage.page.url();
    console.log(`Current URL: ${currentUrl}`);

    // Check if URL contains indicators of customer detail page
    const pageTitle = await followupPastDuePage.page.title();
    console.log(`Page title: ${pageTitle}`);

    // Verify navigation occurred by checking if URL or title contains "Customer" or "Account"
    expect(currentUrl.includes('Customer') ||
      currentUrl.includes('Account') ||
      pageTitle.includes('Customer') ||
      pageTitle.includes('Account')).toBeTruthy();
  } else {
    console.log('Successfully navigated to customer details page');
  }
});
// Test for exporting data
test('Should export data to Excel', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  // The login and navigation to Followup Past Due page is handled by the fixture
...
test('Should refresh the data grid', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  // The login and navigation to Followup Past Due page is handled by the fixture

  // Take screenshot before refreshing
  await followupPastDuePage.takeScreenshot('before-refresh');

  // Get current state of the grid for comparison using page object
  const initialRowCount = await followupPastDuePage.dataGridRows.count();
  console.log(`Initial row count: ${initialRowCount}`);

  // Attempt to capture a timestamp or some identifier from first row using page object
  let initialFirstRowText = '';
  try {
    initialFirstRowText = await followupPastDuePage.dataGridRows.first().locator('.MuiDataGrid-cell').first().textContent();
    console.log(`Initial first row text: ${initialFirstRowText}`);
  } catch (error) {
    console.log('Could not get initial first row text');
  }
  ...
  // Verify grid was refreshed - check row count
  const finalRowCount = await followupPastDuePage.dataGridRows.count();
  console.log(`Final row count after refresh: ${finalRowCount}`);

  // Success condition: the grid is still showing data after refresh
...
test('Should apply Best Fit to columns', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  // The login and navigation to Followup Past Due page is handled by the fixture
...
test.describe('Followup Alert Tests', () => {
  ...
  test('Should navigate to Setup Alerts page', async ({ followupPastDuePageReady, followupPastDuePage }) => {
  ...
  test('Should create a new alert', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // Open Setup Alerts dialog
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
  ...
  test('Should edit an existing alert', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // Open Setup Alerts dialog
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
  ...
  test('Should load a saved alert configuration', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
  ...
  test('Should configure columns for an alert_Naviagte to Alert Columns', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
  ...
  test('Should delete an existing alert', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
  ...
  test('Should set a schedule for an alert', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
  ...
  test('Should process an alert', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
  ...
  test('Should save current alert settings with a name', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
    // - Navigate to Setup Alerts page
    console.log('Opening Setup Alerts dialog...');
    await followupPastDuePage.openSetupAlerts();
...
test.describe('Followup View Settings Tests', () => {
  ...
  test('Should navigate to View Settings page', async ({ followupPastDuePageReady, followupPastDuePage, page }) => {
    // The login and navigation to Followup Past Due page is already handled by the fixture
  ...
  test('Should display existing view settings in the table', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // The login and navigation to Followup Past Due page is already handled by the fixture
  ...
  test('Should save current view settings with a name', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
  ...
  test('Should edit an existing view setting', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    // TO BE IMPLEMENTED
  ...
  test('Should copy an existing view setting', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    /// TO BE IMPLEMENTED
    // - Navigate to View Settings page
    // Open View Settings dialog
    // The login and navigation to Followup Past Due page is already handled by the fixture
  ...
  test('Should load a saved view configuration', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    /// TO BE IMPLEMENTED
    // - Navigate to View Settings page
    // Open View Settings dialog
    // The login and navigation to Followup Past Due page is already handled by the fixture
  ...
  test('Should configure columns for a view', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
    /// TO BE IMPLEMENTED
    // - Navigate to View Settings page
    // Open View Settings dialog
    // The login and navigation to Followup Past Due page is already handled by the fixture
  ...
  test('Should delete an existing view setting', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
   /// TO BE IMPLEMENTED
    // - Navigate to View Settings page
    // Open View Settings dialog
    // The login and navigation to Followup Past Due page is already handled by the fixture
 ...
 */
test('@Regression Followup Past Due_Click on Density option_Compact', async ({followupPastDuePageReady, page, followupPastDuePage }) => {
 ...
 */
test('@Regression Followup Past Due_Click on Density option_Standard', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
 ...
 */
test('@Regression Followup Past Due_Click on Density option_Comfortable', async ({ followupPastDuePageReady, page, followupPastDuePage}) => {
 ...
 */
test('Followup Past Due_Click on BestFit', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
 ...
 */
test('Followup Past Due_Export to Excel', async ({ followupPastDuePageReady, page, followupPastDuePage}) => {
 ...
 */
test('Followup Past Due_Click on Customer Name', async ({ followupPastDuePageReady, page, followupPastDuePage }) => {
