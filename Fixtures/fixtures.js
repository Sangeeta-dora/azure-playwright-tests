...
const env = process.env.ENVIRONMENT || 'devqa'; // Default to qa if not set
const envConfig = config[env] || config.devqa; // Fallback to qa if environment is invalid
...
/**
 * Utility function to safely handle Sara chat popup without causing test failures
 * @param {Object} page - The Playwright page object
 * @param {Object} options - Options for screenshot taking
 * @param {Function} options.takeScreenshot - Optional function for taking screenshots
 * @returns {Promise<boolean>} True if popup was found and closed, false otherwise
 */
async function safeHandleSaraChat(page, options = {}) {
  try {
    console.log("Safely handling Sara chat popup...");
    return await handleSaraChatPopup(page, options);
  } catch (saraChatError) {
    console.warn(`Non-critical Sara chat popup handling error: ${saraChatError.message}`);
    console.log("Continuing test execution despite Sara chat popup handling error");
    return false;
...
// Export the safe Sara chat handling utility function
exports.safeHandleSaraChat = safeHandleSaraChat;

/**
 * Sets up automatic Sara chat popup handling for the entire test
 * This function will monitor page changes and automatically close the popup when detected
 * @param {Object} page - The Playwright page object
 * @param {Object} options - Options for handling the popup
 * @returns {Promise<Function>} Function to stop the auto handler
 */
async function setupAutoSaraChatHandler(page, options = {}) {
  console.log("Setting up automatic Sara chat popup handling for the entire test...");
  const { setupSaraChatAutoHandler } = require('../PageObjects/handleSaraChatPopup');
  
  try {
    return await setupSaraChatAutoHandler(page, options);
  } catch (error) {
    console.warn(`Error setting up auto Sara chat handler: ${error.message}`);
    console.log("Tests will continue without automatic Sara chat handling");
    // Return a no-op cleanup function
    return async () => { console.log("No auto Sara handler to clean up"); };
  }
}

// Export the auto handler setup function
exports.setupAutoSaraChatHandler = setupAutoSaraChatHandler;
